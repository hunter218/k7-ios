...
                        <view class="flex-item" style="width: 20%;">
							<view class="iconfont icon-LUCKYSB59dce4f8"></view>
						</view>
                        <view class="flex-item" style="width: 30%;line-height: 120rpx;font-weight:bold">极速飞艇</view>
				...
				<view class="flex-item flex-item-V" style="background-color: #8b8b8b;">
...
<script>
	...
	export default {
		...
		methods:{
			hallMessageJsft: function (msg) {
				if (msg.status == 'success' && msg.result.type == 'jsftTictok') {
					let data = msg.result
		...
		created() {
			clearInterval(this.tictokTimer)
			uni.$on('hallMessageJsft', (msg)=>{
				this.hallMessageJsft(msg);
			})
			this.initTictok()
		},
		destroyed() {
			clearInterval(this.tictokTimer)
			uni.$off('hallMessageJsft')
		}
	}
</script>

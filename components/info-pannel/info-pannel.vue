		...
		<view class="info-pannel">
			<view class="uni-flex uni-row" :class="(open ? 'gSbRmf' : 'dRtNjK')">
				...
				<view class="flex-item has-left-border">
					<view class="info-title">可退水积分</view>
					<view class="info-content">{{ unsettledWater }}</view>
				</view>
				<view class="flex-item has-left-border">
					<view class="info-title">今日总流水</view>
					<view class="info-content">{{ todayTotalAmount }}</view>
				</view>
				<view class="flex-item has-left-border">
					<view class="info-title">今日输赢</view>
...
<script>
	import {getMyQuota} from '../../common/api/index.js';
	export default {
		data() {
			return {
				open: true,
		...
		methods: {
			...
			refreshQuota() {
				...
				getMyQuota({'Authorization': 'Bearer ' + token}).then(data => {
					...
					this.unsettledWater = data.unsettled_water

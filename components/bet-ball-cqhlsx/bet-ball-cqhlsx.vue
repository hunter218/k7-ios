			...
			<uni-collapse-item :title=index :open=true :disabled=clickDisabled>
				<view class="ball-btns-wrapper">
					...
					@click="change(index, key)" v-bind:class="{ active: ball.isActive }">
						<view class="cqhlsx-ball ball-0" v-if="ball.val==0">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-1" v-if="ball.val==1">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-2" v-if="ball.val==2">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-3" v-if="ball.val==3">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-4" v-if="ball.val==4">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-5" v-if="ball.val==5">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-6" v-if="ball.val==6">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-7" v-if="ball.val==7">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-8" v-if="ball.val==8">{{ ball.val }}</view>
						<view class="cqhlsx-ball ball-9" v-if="ball.val==9">{{ ball.val }}</view>
						<view class="text" v-if="isNaN(ball.text)">{{ ball.text }}</view>

		...
		<view class="bill-pannel">
			...
			<view>
				<view class="uni-flex uni-row" v-for="(item, index) in list" :key="index">
					...
					<view class="flex-item-v width-4">{{ parseFloat(item.settled_amount).toFixed(2) }}</view>
				</view>
...
<script>
	import {getSettled} from '../../common/api/index.js'
	...
	export default {
		...
		onLoad() {
			...
			getSettled({'Authorization': 'Bearer ' + token}).then(data => {
